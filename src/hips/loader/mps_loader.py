...
def load_mps_primitive(mip_model : MIPModel, path):
    ...
    :return: None
    """
    name, objective_name, row_names, col_names, col_types, types, c, A, rhs_names, rhs, bnd_names, bnd = loader.load_mps(path)
    # model variables
    n = len(col_names)
    x = []
...
def load_mps_advanced(mip_model : MIPModel, path):
    ...
    for i in range(n):
        ...
        if col_types[i] == 'continuous':
            ...
            if lb == 0 and ub == 1:
                ...
                int_indices += [i]
    empty = {"cont": True if len(cont_indices) == 0 else False, "int": True if len(int_indices) == 0 else False, "bin": True if len(bin_indices) == 0 else False}
    ...
    x_bin = mip_model.add_variable(name="bin", var_type=VarTypes.BINARY, dim=len(bin_indices)) if not empty["bin"] else None
    # objective function
...
if __name__ == "__main__":
    ...
    print("Primitive Load: {}s loading time, {}s optimization time, objective value: {}\nAdvanced Load: {}s loading time, {}s optimization time, objective value: {}"

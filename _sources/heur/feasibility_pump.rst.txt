...
.. code-block:: none
    ...
    do
        ...
        :math:`\bar{x}` = argmin{:math:`\Delta(x, \tilde{x}), Ax \le b}`
    while(:math:`\tilde{x}` infeasible)
...
In case we reach a cycle of length 1 (:math:`\tilde{x}^i` = :math:`\tilde{x}^{i+1}`) the algorithm, the
T binary variables of the current integer solution :math:`\tilde{x}` with highest distance :math:`|x*_j - \tilde{x}_j|` are flipped and fixed for the calculation of the next
:math:`\bar{x}`. The number T of variables to flip is uniformally chosen from the range :math:`( \lfloor \frac{t}{2} \rfloor , \lfloor 1.5*t \rfloor )`.
...
if :math:`|x*_j - \tilde{x}_j| + max(0, {\rho}_j) > 0.5`.
